...
export const markNotificationUnread = async (req, res, next) => {
  try {
    const userId = req.user.id || req.user.userId;
    const updated = await notificationService.markAsUnread(userId, req.params.id);
    if (!updated) {
      return errorResponse(res, 404, 'Notification not found');
    }
...
export const deleteNotification = async (req, res, next) => {
  try {
    ...
    const notification = await notificationService.softDeleteNotification(
      req.params.id,
      userId,      // ownership: only owner can delete
      deletedRemarks,
      userId,      // updated_by = caller
      true,             // ownerOnly = true
    );
    if (!notification) {
...
export const bulkDeleteNotifications = async (req, res, next) => {
  try {
    ...
    const result = await notificationService.bulkSoftDeleteNotifications(
      ids,
      userId,      // ownership: filter to caller's notifications only
      deletedRemarks,
      userId,
    );
