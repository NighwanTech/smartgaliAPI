...
export const createReview = async (req, res, next) => {
  try {
    const data = { ...req.body };
    if (!data.user_id && (req.user?.id || req.user?.userId)) {
      data.user_id = req.user.id || req.user.userId;
    }
    const review = await businessReviewService.createReview(data);
...
export const replyReview = async (req, res, next) => {
  try {
    const { replyText, response, reply } = req.body || {};
    const replyContent = replyText || response || reply;
    const review = await businessReviewService.updateReview(req.params.id, { remark: replyContent });
    if (!review) {
      return errorResponse(res, 404, 'Business review not found');
    }
    return successResponse(res, 200, 'Owner response posted successfully', review);
  } catch (error) {
    next(error);
  }
};
